...
#     cv2.imwrite('result_mask.png',result_mask)
#     # Take the coordinates from this resulted_mask image and then fill this rectangle coordinates with a 255 in the original mask image
...
def draw_bounding_boxes(mask, bounding_boxes, image):
    # Threshold the mask to get binary foreground
    _, mask_binary = cv2.threshold(mask, 25, 255, cv2.THRESH_BINARY)
    
    # Create a blank mask to draw bounding boxes
    ...
    # Find intersection between mask and bounding box mask
    result_mask = cv2.bitwise_and(mask_binary, bbox_mask)
    
    # Convert result_mask to binary (CV_8UC1)
    ...
    # Draw filled contours on the original mask
    for contour in contours:   
        # Fill the corresponding region in the mask with the colors from the extracted region
        cv2.fillPoly(mask_binary, [contour], (255,255,255))
...
mask_image_path = '/home/usama/Median_erode__contours_low__and_high_resolution_test/LUV_Meanshift_Bandwidth_8_fahad_results/0001/0001_1.jpg'  # Replace with your mask image path
mask = cv2.imread(mask_image_path)
image_path = '/home/usama/Median_erode__contours_low__and_high_resolution_test/LUV_Meanshift_Bandwidth_8_fahad_results/fahad_Data_source_masks/fahad_data/0001.jpg'
...
cv2.imwrite('result_0001_1.png', mask)
